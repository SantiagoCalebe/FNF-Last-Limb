...
class CreditsMenuState extends MusicBeatState {
    ...
    override public function create():Void {
        ...
        instructionsText = new FlxText(10, 10, FlxG.width - 20, "Press ESC to skip");
        instructionsText.setFormat("VCR OSD Mono", 16, FlxColor.WHITE, "bottom");
        add(instructionsText);

        instructionsText2 = new FlxText(10, 30, FlxG.width - 20, "Press G to enter Github REPO.");
    ...
    override public function update(elapsed:Float):Void {
        ...
        handleMouseScroll();
        clampCreditsPosition();
    }

    private function handleMouseScroll():Void {
        if (FlxG.mouse.wheel != 1) {
            creditsText.y += FlxG.mouse.wheel * -10; // Ajuste para suavidade
        }
    }

    private function clampCreditsPosition():Void {
        if (creditsText.y > FlxG.height) {
            creditsText.y = FlxG.height;
        }
        if (creditsText.y < -creditsText.height + 50) { // 50 para evitar corte brusco
            creditsText.y = -creditsText.height + 50;
        }
    }
}
